    ...
    for task in mb.tasks:
        ...
        if not classification:
            ...
            for ii, fold in enumerate(task.folds):
                ...
                # keep test set intact
                val_df = train_df[0 : len(test_df)]
                train_df.to_csv(f"train_fold_{ii}.csv", index=False)
                val_df.to_csv(f"val_fold_{ii}.csv", index=False)
                test_df.to_csv(f"test_fold_{ii}.csv", index=False)

                print(f"Saved train/val/test CSVs for fold {ii}:")
                ...
                cmd = (
                    "CUDA_VISIBLE_DEVICES=3 python -m alignn.train_alignn --root_dir "
                    + fold_name
                    + " --config "
                    + fold_name
                    + "/"
                ...
                print(f"Dataset_name: {task.dataset_name}, Fold: {fold}, MAE: {mae}, R2: {r2}")
            maes = np.array(maes)
            print("5-fold mae", np.mean(maes), np.std(maes))
            r2s = np.array(r2s)
            print("5-fold r2", np.mean(r2s), np.std(r2s))
            print()
            print()
            print()
...
if __name__ == "__main__":
    ...
    mb.to_file(f"{task_name}_results.json.gz")